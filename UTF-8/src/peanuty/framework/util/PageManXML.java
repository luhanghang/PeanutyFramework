...
public class PageManXML {
	...
	public static String getResultListXML(Map reqH, DBConn dbc) throws Exception{
		int jumpPage = 1;
		if (reqH.get(PageMan.JUMP_PAGE) != null) {
			jumpPage = (Integer.parseInt(reqH.get(PageMan.JUMP_PAGE) + ""));
		}
		if (reqH.get(PageMan.ROWS_PER_PAGE) != null) {
			return getResultListXML(reqH.get(PageMan.SQL) + "",jumpPage, Integer.parseInt(reqH.get(PageMan.ROWS_PER_PAGE) + ""), dbc);
		}
		return getResultListXML(reqH.get(PageMan.SQL) + "", jumpPage, dbc);
	}
	...
	public static String getXMLResultList(Map reqH, DBConn dbc) throws Exception{
		...
		if (reqH.get(PageMan.ROWS_PER_PAGE) != null) {
			return getXMLResultList(reqH.get(PageMan.SQL) + "",jumpPage, Integer.parseInt(reqH.get(PageMan.ROWS_PER_PAGE) + ""), dbc);
		}
		return getXMLResultList(reqH.get(PageMan.SQL) + "", jumpPage, dbc);
	}
}